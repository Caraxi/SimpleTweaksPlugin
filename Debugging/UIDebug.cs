...
public unsafe class UIDebug : DebugHelper {
        ...
        if (atkUnitBase->CollisionNodeListCount > 0) {
            ...
            if (ImGui.TreeNode($"Collision List##{(ulong)atkUnitBase:X}")) {
                ...
                for (var j = 0; j < atkUnitBase->CollisionNodeListCount; j++) {
                    PrintNode(atkUnitBase->CollisionNodeList[j], false, $"[{j}] ");
                }
    ...
    private static string NodeID(uint id, bool includeHashOnNoMatch = true) {
        ...
        if (!CustomNodes.TryGet(id, out var customNodeName)) {
            customNodeName = customNodeIds.TryGetValue(id, out var nodeId) ? nodeId : string.Empty;
        }
    ...
    private static void PrintResNode(AtkResNode* node) {
        ...
        ImGui.Text($"X: {node->X} Y: {node->Y} " + $"ScaleX: {node->ScaleX} ScaleY: {node->ScaleY} " + $"Rotation: {node->Rotation} " + $"Width: {node->Width} Height: {node->Height} " + $"OriginX: {node->OriginX} OriginY: {node->OriginY} " + $"Priority: {node->Priority}, Depth: {node->Depth}/{node->Depth_2} " + $"Flags: {node->NodeFlags} / {node->DrawFlags:X} " + $"DrawFlags: {node->DrawFlags:X}");
        ImGui.Text($"RGBA: 0x{node->Color.R:X2}{node->Color.G:X2}{node->Color.B:X2}{node->Color.A:X2} " + $"AddRGB: {node->AddRed} {node->AddGreen} {node->AddBlue} " + $"MultiplyRGB: {node->MultiplyRed} {node->MultiplyGreen} {node->MultiplyBlue}");
    ...
    private static void DrawAtkTimeline(AtkResNode* node, AtkTimeline* timeline) {
        ImGui.Text($"Parent Time: {timeline->ParentFrameTime:0.00} ({(ushort)(timeline->ParentFrameTime * 30)}) " + $"Frame Time: {timeline->FrameTime:0.00} ({(ushort)(timeline->FrameTime * 30)}) " + $"Animated Properties: {~timeline->Mask} Mask2: {timeline->Flags}");
        ...
        if (prop->Type != AtkTimelineKeyGroupType.None) {
            if (ImGui.TreeNode($"{name} - {prop->KeyFrameCount} Keyframes of {prop->Type}##{(ulong)node:X}_{(ulong)prop:X}")) {
                for (var keyframeIdx = 0; keyframeIdx < prop->KeyFrameCount; ++keyframeIdx) {
                    ...
                    ImGui.Text($"Frame: {keyframe.FrameIdx} " + $"Interpolation: {keyframe.Interpolation} " + $"({keyframe.SpeedCoefficient1:0.##}, {keyframe.SpeedCoefficient2:0.##})");
                    ...
                    ImGui.Text("Value: " + prop->Type switch {
                        ...
                        AtkTimelineKeyGroupType.NodeTint => $"Multiply: ({v.NodeTint.MultiplyRGB.R}, {v.NodeTint.MultiplyRGB.G}, {v.NodeTint.MultiplyRGB.B}) " + $"Add: ({v.NodeTint.AddR}, {v.NodeTint.AddG}, {v.NodeTint.AddB})",
