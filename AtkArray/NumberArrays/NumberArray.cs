...
public unsafe abstract class NumberArray<T>(int numberArrayIndex, string name) : NumberArray(numberArrayIndex, name) where T : unmanaged, INumberArrayStruct {
    public T* Data => (T*)BaseData->IntArray;
}

public unsafe abstract class NumberArray(int numberArrayIndex, string? name = null) {
