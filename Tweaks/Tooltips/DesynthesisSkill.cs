        ...
        if (id < 2000000) {
            ...
            if (nullableItem is { Desynth: > 0 }) {
                ...
                if (seStr is { Payloads.Count: > 0 }) {
                    ...
                    var textPayload = seStr.Payloads.OfType<TextPayload>().Where(p => p.Text != null).LastOrDefault(p => p.Text != null && (p.Text.Contains($": {item.LevelItem.RowId},00") || p.Text.Contains($": {item.LevelItem.RowId}.00") || p.Text.Contains($":{item.LevelItem.RowId}.00") || p.Text.Contains($"{item.LevelItem.RowId},00]")));
                    if (textPayload != null) {
