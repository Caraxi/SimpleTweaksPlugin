...
namespace SimpleTweaksPlugin.Tweaks.UiAdjustment
{
    public unsafe class PreciseHpPercent : UiAdjustments.SubTweak
    {
        ...
        private void Update(bool reset = false)
        {
            ...
            {
                if (target is not Character || ((Character)target).MaxHp == 0) reset = true;
            ...
            {
                if (External.Targets?.FocusTarget is not Character
                    || ((Character)(External.Targets?.FocusTarget)).MaxHp == 0)
                    reset = true;
                var ui = (AtkUnitBase*)External.GameGui.GetAddonByName("_FocusTargetInfo", 1);
        ...
        private void UpdateHpName(string uiObjectName, int gaugeBarIndex, int hpNodeIndex, int nameNodeIndex, int expectedUldNodelistLen, GameObject target, bool reset)
        {
            var ui = (AtkUnitBase*)External.GameGui.GetAddonByName(uiObjectName, 1);
            if (ui != null && (ui->IsVisible || reset))
            {
        ...
        private void UpdateFocusTargetHp(AtkComponentNode* unitBase, AtkTextNode* cloneTextNode, GameObject target, bool reset = false)
        {
            var textNode = UpdateBase(unitBase, cloneTextNode, FocusTargetNameHpNodeID, target, 0, reset, false);
            if (textNode == null || reset || target is not Character chara) return;
        ...
        private void UpdateHpPercent(AtkComponentNode* unitBase, AtkTextNode* originalNode, GameObject target, uint NodeID, bool reset = false)
        {
            var textNode = UpdateBase(unitBase, originalNode, NodeID, target, 4, reset);
            if (target == null || reset || target is not Character chara) return;
        ...
        private void UpdateTargetName(AtkComponentNode* unitBase, AtkTextNode* cloneTextNode, GameObject target, uint NodeID, bool reset = false)
        {
        ...
        private string FormatHp(Character chara, int precision)
        {
        ...
        private AtkTextNode* UpdateBase(AtkComponentNode* gauge, AtkTextNode* cloneTextNode, uint NodeID, GameObject target, int startSearchIndex, bool reset = false, bool setPosition = true)
        {
            ...
            if (target is Character chara) return textNode;
            textNode->SetText("");
