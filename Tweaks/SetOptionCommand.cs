...
public unsafe class SetOptionCommand : CommandTweak
{
    ...
    public class OptionDefinition<T> : IOptionDefinition
    {
        ...
        public OptionDefinition(string name, string id, OptionGroup group,
            Func<(Dictionary<string, T>, Dictionary<string, T>)> valuesFunc, params string[] alias)
        {
            this.Name = name;
            this.ID = id;
    ...
    private readonly List<IOptionDefinition> optionDefinitions = new() {
        new OptionDefinition<uint>("GamepadMode", "PadMode", OptionGroup.UiConfig, ValueType.Boolean, "gp")
            { AllowToggle = true },
        new OptionDefinition<uint>("ItemTooltips", "ItemDetailDisp", OptionGroup.UiControl, ValueType.Boolean, "itt")
            { AllowToggle = true },
        new OptionDefinition<uint>("ActionTooltips", "ActionDetailDisp", OptionGroup.UiControl, ValueType.Boolean,
            "att") { AllowToggle = true },
        new OptionDefinition<uint>("LegacyMovement", "MoveMode", OptionGroup.UiControl, ValueType.Boolean, "lm")
            { AllowToggle = true },
        new OptionDefinition<uint>("HideUnassignedHotbarSlots", "HotbarEmptyVisible", OptionGroup.UiConfig,
            ValueType.Boolean, "huhs") { AllowToggle = true },
        new OptionDefinition<uint>("LimitMouseToGameWindow", "MouseOpeLimit", OptionGroup.System, ValueType.Boolean,
            "lmtgw") { AllowToggle = true },

        new OptionDefinition<uint>("OwnDisplayName", "NamePlateDispTypeSelf", OptionGroup.UiConfig,
            ValueType.NamePlateDisplay, "odn") { AllowToggle = true },
        new OptionDefinition<uint>("PartyDisplayName", "NamePlateDispTypeParty", OptionGroup.UiConfig,
            ValueType.NamePlateDisplay, "pdn") { AllowToggle = true },
        new OptionDefinition<uint>("AllianceDisplayName", "NamePlateDispTypeAlliance", OptionGroup.UiConfig,
            ValueType.NamePlateDisplay, "adn") { AllowToggle = true },
        new OptionDefinition<uint>("OtherPlayerDisplayName", "NamePlateDispTypeOther", OptionGroup.UiConfig,
            ValueType.NamePlateDisplay, "opcdn") { AllowToggle = true },
        new OptionDefinition<uint>("FriendDisplayName", "NamePlateDispTypeFriend", OptionGroup.UiConfig,
            ValueType.NamePlateDisplay, "fdn") { AllowToggle = true },
        ...
        new OptionDefinition<uint>("DirectChat", "DirectChat", OptionGroup.UiControl, ValueType.Boolean, "dc")
            { AllowToggle = true },
    };
        ...
        if (optionDefinition.AllowToggle && inputValue is "t" or "toggle") {
            switch (optionDefinition) {
                case OptionDefinition<uint> i: {
                    if (!optionSection.TryGetProperties(optionDefinition.ID, out UIntConfigProperties properties) ||
                        properties == null) {
                        Plugin.Error(this, new Exception($"Failed to get option detail for {optionDefinition.Name}"),
                            allowContinue: true);
                        return;
                    }
                    ...
                    var valueName =
                        i.Values.Where(kvp => kvp.Value == toggleValue).Select(kvp => kvp.Key).FirstOrDefault() ??
                        i.ValueAlias.Where(kvp => kvp.Value == toggleValue).Select(kvp => kvp.Key).FirstOrDefault() ??
                        $"{toggleValue}";
