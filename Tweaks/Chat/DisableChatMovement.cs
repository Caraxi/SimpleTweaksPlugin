        ...
        public override void Setup() {
            ...
            try {
                if (setUiPositionAddress == IntPtr.Zero) {
                    setUiPositionAddress = External.SigScanner.ScanText("40 53 48 83 EC 20 80 A2 ?? ?? ?? ?? ??");
                }

                if (chatPanelControlAddress == IntPtr.Zero) {
                    chatPanelControlAddress = External.SigScanner.ScanText("40 55 57 48 81 EC ?? ?? ?? ?? 48 8B F9 45 8B C8");
                }
