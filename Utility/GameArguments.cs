...
public unsafe class GameArguments {
    
    [DllImport("kernel32.dll", CharSet = CharSet.Auto)]
    private static extern System.IntPtr GetCommandLine();
    
    private static GameArguments _instance;
    public static GameArguments Instance => _instance ??= new GameArguments();
    
    public string GamePath { get; } = string.Empty;
    public string UserPath { get; } = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "My Games", "FINAL FANTASY XIV - A Realm Reborn");

    private GameArguments() {
        var cmdLinePtr = (ushort*) GetCommandLine();
        var length = 0;
        while (cmdLinePtr != null && cmdLinePtr[length] != 0) length += 1;
        var cmdLine =  Encoding.Unicode.GetString((byte*)cmdLinePtr, length * 2);
        var regex = new Regex(@"\S+|""[\S\s]*""");
        var x = regex.Matches(cmdLine).Cast<Match>().Select(m => m.Value).ToArray();
        var arguments = ParseEncryptedCommandLine(x);
        
        GamePath = arguments[0].Trim();
    ...
    private static bool TryDecrypt(byte[] encrypted, uint key, out string decrypted) {
        var blowfish = new LegacyBlowfish(GetKeyBytes(key));

        try {
            var decryptedBytes = encrypted;
            blowfish.Decrypt(ref decryptedBytes);
            decrypted = Encoding.UTF8.GetString(decryptedBytes);
            if (decrypted[0] != ' ') return false;
            if (decrypted[1] != '/') return false;
            if (!decrypted.Contains("/DEV.TestSID =")) return false;
            return true;
        } catch {
            //
        }
    ...
    private static bool TryDecryptRange(byte[] encrypted, uint keyStart, uint keyCount, char checksum, out string decrypted) {
        var i = 0;
        var skipped = 0;
        do {
            var key = DeriveKey(keyStart++);
            if (DeriveChecksum(key) != checksum) {
                skipped++;
                continue;
            }
            i++;
            if (TryDecrypt(encrypted, key, out decrypted)) {
                return true;
            }
        } while (keyCount-- > 0);
    ...
    private static uint DeriveKey(uint rawTickCount)
    {
    ...
    private static byte[]  FromMangledSeB64(string input) {
        return Convert.FromBase64String(input.Replace('-', '+').Replace('_', '/').Replace('*', '='));
    }
    
    private string[] ParseEncryptedCommandLine(string[] args) {
        var parsedArgs = new List<string>();
        foreach (var arg in args) {
            if (arg.StartsWith("//**sqex0003") && arg.EndsWith("**//")) {
                var startTime = Process.GetCurrentProcess().StartTime.Ticks / TimeSpan.TicksPerMillisecond;
                var envStartTime = (DateTime.Now.Ticks / TimeSpan.TicksPerMillisecond) - Environment.TickCount;
                var processStartTime = startTime - envStartTime;
                var checksum = arg[^5];
                var ciphertext = FromMangledSeB64(arg[12..^5]);
                if (GameArguments.TryDecryptRange(ciphertext, (uint) processStartTime - 1000, 2000, checksum, out var decrypted)) {
                    var splitArgs = decrypted.Split(" /").Select(s => s.Replace(" =", "=").Trim().Trim('\0')).Where(s => s.Length > 0);
                    parsedArgs.AddRange(splitArgs);
                } else {
                    parsedArgs.Add(arg);
                }
